...
import AddProductsMapper from "../../components/managements/AddProductsMapper";
import ProductsMapperCard from "../../components/managements/ProductsMapperItemCard";
import { useProductsMapper } from "../../hooks/useProductsMapper";

const ProductsMapper = () => {
  const productsMapperStore = useProductsMapper();

  const [isCreateProductsMapperOpen, setIsCreateProductsMapperOpen] =
    useState(false);
  ...
  return (
    <main>
        ...
        <div className="w-full mb-1">
          ...
          <div className="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700">
              ...
              <div className="flex items-center w-full sm:justify-end">
                <div className="flex pl-2 space-x-1 justify-center items-center">
                  <input
                    ...
                    placeholder="Search for products mapper"
                  />
                </div>
              </div>
            </div>
            <button
              ...
              onClick={() => setIsCreateProductsMapperOpen(true)}
            >
              Add new products mapper
            </button>
          </div>
        </div>
      </div>
      <div className="overflow-x-auto grid grid-cols-3 p-4 gap-4">
        {productsMapperStore.data.map((productMapper) => (
          <ProductsMapperCard
            key={productMapper.id}
            productsMapper={productMapper}
          />
        ))}
        {productsMapperStore.loading && (
      ...
      <AddProductsMapper
        isCreateProductsMapperOpen={isCreateProductsMapperOpen}
        setIsCreateProductsMapperOpen={setIsCreateProductsMapperOpen}
      />
    </main>
  );
};
