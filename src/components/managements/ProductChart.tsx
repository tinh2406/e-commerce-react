...
import { ProductStatisticsService } from "../../services/statistics.service";
...
const ProductChart = () => {
  const [products, setProducts] = useState<StatisticResponse>([]);
  const [growth, setGrowth] = useState<StatisticResponse>([]);
  const [productQuery, setProductQuery] = useState<StatisticQuery>({
    cycle: "week",
    num_cycle: 7,
  });
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      const res = await ProductStatisticsService(productQuery);
      if (res.length > 0) {
        setProducts(res);
      ...
      <div className="w-full flex justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative border rounded-md">
            ...
            >
              <span className="capitalize">Every {productQuery.cycle}</span>
              <div>
                <svg
            ...
            xaxis: {
              categories: [
                ...products.map((product) =>
                  product.range_label == "0" ? "Other" : product.range_label
                ),
              ],
            },
          }}
